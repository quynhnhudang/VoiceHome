...
from environmental_manager import get_current_weather  # Assume this file is defined with a get_current_weather function
    ...
    while True:
        ...
        if command:
            if 'bật đèn' in command:
                control_light(light_id=1, state=True)  # Example: Turn on light
            elif 'tắt đèn' in command:
                control_light(light_id=1, state=False)  # Example: Turn off light
